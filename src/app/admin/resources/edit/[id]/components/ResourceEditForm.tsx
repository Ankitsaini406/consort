...
const handleResourceEditSubmit = async (
    ...
    const sanitizedData = sanitizeResourceData(data);

    try {
        await withRateLimit(clientId, 'formSubmission', async () => {
            console.log(`[${context}] Edit submit started:`, { isDraft, documentId });

            // 🔍 DEBUG: Log user object structure in resource edit
            console.log('🔍 DEBUG: User object in resource edit:', {
                user: user,
                userName: user?.name,
            ...
            if (sanitizedData.resourceTitle === "force_error") {
                throw new Error("This is a simulated API error during resource update.");
            }
            ...
            const newSectionImages = (sanitizedData.sections || [])
            ...
            let urlIndex = 0;
            const sectionUploads = (sanitizedData.sections || []).map((section, index) => ({
                id: section.id,
            ...
            const updateData: Partial<ResourceFormData & { heroImageUrl: string | null }> = {
            ...
            const finalResourceData = updateData as Record<string, unknown>;
            delete finalResourceData.numberOfSections;

            await updateDoc(doc(db, "resources", documentId), finalResourceData as any);
            ...
            try {
                ...
                const pathsToRevalidate = getResourceRevalidationPaths(sanitizedData.resourceType, documentId);
                ...
                // Don't throw - the resource was updated successfully, revalidation is just optimization
            }
...
export const ResourceEditForm: React.FC<ResourceEditFormProps> = ({ documentId }) => {
    ...
    const baseConfig = resourceFormConfig(globalTags, clients, industries);
